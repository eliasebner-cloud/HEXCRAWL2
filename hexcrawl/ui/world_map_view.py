...
class WorldMapView:
    ...
    def _draw_debug_panel(self, screen: pygame.Surface) -> None:
        ...
        selected_hex = self._format_hex(self.selected_hex)
        hover_hex = self._format_hex(self.hover_hex)

        lines = [
            "Mode: World",
            f"Selected hex: {selected_hex}",
            f"Hover hex:    {hover_hex}",
    ...
    @staticmethod
    def _format_hex(value: tuple[int, int] | None) -> str:
        if value is None:
            return "(none)"
        q, r = value
        return f"({q},{r})"
